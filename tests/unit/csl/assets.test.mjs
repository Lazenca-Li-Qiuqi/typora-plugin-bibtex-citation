...
test("ensureCslTemplate 优先使用当前 Markdown frontmatter 中的 csl", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "csl-assets-frontmatter-"));
  const notePath = path.join(tempDir, "paper.md");
  const stylePath = path.join(tempDir, "frontmatter-apa.csl");
  const original = fs.readFileSync(path.join(styleRoot, "apa.csl"), "utf8");
  fs.writeFileSync(stylePath, original, "utf8");
  ...
  const { ensureCslTemplate } = await import(createFreshModuleUrl("src/csl/assets.js"));
  const templateName = ensureCslTemplate({
    window: {
      editor: {
        getMarkdown() {
          return `---
csl: ./frontmatter-apa.csl
---
...
正文`;
  ...
  assert.match(templateName, /frontmatter-apa/);
});
