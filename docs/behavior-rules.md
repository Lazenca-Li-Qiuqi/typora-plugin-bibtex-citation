...
- 在以下两类上下文中触发建议：
  ...
  - 独立正文位置的 `@smith`
  - `根据 @smith`
- `根据@smith`、邮箱、URL、代码与方括号之外的非独立 `@token` 不触发叙述式建议。
...
- 当前文档状态与 CSL 工作流统一扫描严格闭合方括号、叙述式 citation 和受控 citation 真源。
- 扫描只识别同一行内的闭合方括号块，不跨行拼接。
- HTML 注释 `<!-- ... -->` 中的 `[@key]` 会被整体忽略。
- 叙述式扫描采用 Pandoc key 词法：key 以字母、数字或 `_` 开头，内部只接受单个 `:.#$%&-+?<>~/` 标点；特殊 key 可写成 `@{key}`。
- 叙述式扫描排除 YAML frontmatter、HTML 注释、HTML 标签、行内代码、围栏代码、缩进代码和所有方括号上下文。
- 只有存在于当前合并 BibTeX 文献库中的独立裸 key 才会成为叙述式引用；未知 `@token` 保持普通文本，避免误判账号等内容。
...
- 统计会同时识别：
  - 正文里可见的严格 `[@key]` / `[@a; @b]`
  - 正文里可见的叙述式 `@key`
  - 受控 citation 注释中保存的原始 `[@key]` 或 `@key`
...
正文中独立出现且存在于当前文献库的 `@key` 会作为 Pandoc 风格叙述式 citation source，使用 CSL composite 模式渲染为类似 `Smith (2024)` 的结果。叙述式语法不会放宽上述方括号规则。
...
- 统一引用源同时识别：
  - 正文里严格合法的 `[@key]`
  - 正文里独立且 key 已知的叙述式 `@key`
  - 受控 citation 块中保存的原始 `[@key]` 或 `@key`

## 6. 受控 citation 规则

### 6.1 真源原则

- 受控 citation 块中的原始 `[@key]` / `@key` 是长期持久真源。
